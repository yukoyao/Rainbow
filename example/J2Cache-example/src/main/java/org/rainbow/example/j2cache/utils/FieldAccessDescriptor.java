...
public class FieldAccessDescriptor {
  ...
  private FieldAccessDescriptor(String fieldPath, boolean isFirst) {
    try {
      ...
      List<Integer> indexList = Arrays.asList(bracketIndex, commaIndex, optionalIndex).stream().filter(i -> i > -1)
  ...
  private void fixPath() {
    if (hasNext()) {
      ...
      this.nextFieldAccessDescriptor.currentPath =
          this.currentPath + separator + this.nextFieldAccessDescriptor.currentPath;
